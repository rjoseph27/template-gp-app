...
/**
 * @class CarrierOrdersComponent
 * @description The component for the gp orders
 */
@Component({
    selector: 'carrier-orders',
    templateUrl: './gp-orders.component.html',
    styleUrls: ['../base-order.component.scss']
  })
  export class CarrierOrdersComponent extends baseOrdersComponent {
    /**
        ...
        switch (row.status) {
          case ItemsStatus.WAIT_CONFIRMATION:
            return {
              ...
              icon: 'check_circle',
              action: () => this.router.navigate([CarrierRoutes.confirmOrders.fullPath()], { queryParams: {
                id: row.id,
                status: row.status,
                from: row.route.from,
                to: row.route.to,
          ...
          case ItemsStatus.WAITING_RECEPTION:
            return {
              ...
              icon: 'local_shipping',
              action: () => this.router.navigate([CarrierRoutes.itemOnHisWay.fullPath()], { queryParams: {
                id: row.id,
                status: row.status,
                from: row.route.from,
                to: row.route.to,
          ...
          case ItemsStatus.AT_CHECKPOINT:
            return {
              ...
              icon: 'location_on',
              action: () => this.router.navigate([CarrierRoutes.itemAtCheckpoint.fullPath()], { queryParams: {
                id: row.id,
                status: row.status,
                from: row.route.from,
                to: row.route.to,
          ...
          case ItemsStatus.READY_TO_PICK_UP:
            return {
              ...
              icon: 'concierge',
              action: () => this.router.navigate([CarrierRoutes.readyForPickup.fullPath()], { queryParams: {
                id: row.id,
                status: row.status,
                from: row.route.from,
                to: row.route.to,
          ...
          case ItemsStatus.WITH_GP:
            return {
              ...
              action: () => this.router.navigate([CarrierRoutes.gpTracking.fullPath()], { queryParams: {
                id: row.id,
                status: row.status,
                from: row.route.from,
                to: row.route.to,
          ...
          case ItemsStatus.FINALIZED:
            return {
              ...
              icon: 'done',
              action: () => this.router.navigate([CarrierRoutes.itemDeliveredGp.fullPath()], { queryParams: {
                id: row.id,
                status: row.status,
                from: row.route.from,
                to: row.route.to,
